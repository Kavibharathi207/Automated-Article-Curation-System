<script>
  // @ts-nocheck
  import { currentPage, scheduledPosts, interestedBlogs, bookmarks, pipelineStats } from '../stores/store.js';
  ...
  const stats = [
    { label: 'Total views',    value: '12,840', delta: '+8% this month',  up: true  },
    { label: 'Reads',          value: '4,210',  delta: '+12% this month', up: true  },
    { label: 'Followers',      value: '318',    delta: '+5 this week',    up: true  },
    { label: 'Claps received', value: '1,092',  delta: '-3% this month',  up: false },
  ];

  const barData = [40,65,50,80,70,90,60,75,85,55,95,100,70,80,60,75,85,90,65,70,80,75,60,85,90,95,70,80,65,75];

  // Use published posts for top stories; fall back to mockBlogs
  $: topStories = (() => {
    const pub = $scheduledPosts.filter(p => p.status === 'published').slice(0, 5);
    if (pub.length >= 3) return pub;
    return mockBlogs.slice(0, 5);
  })();

  const mockViews = [12840, 8420, 6310, 4980, 3760];
</script>
...
<div class="stats-wrap">
  ...
  <!-- Audience stats -->
  <div class="section-head">Audience</div>
  <div class="stats-grid">
    {#each stats as st}
      <div class="stat-card">
        <div class="stat-value">{st.value}</div>
        <div class="stat-label">{st.label}</div>
        <div class="stat-delta" class:up={st.up} class:down={!st.up}>{st.delta}</div>
      </div>
    {/each}
  </div>
  ...
  <div class="story-list">
    {#each topStories as blog, i}
      <div class="story-row">
        {#if blog.image}<img src={blog.image} alt="" class="story-thumb" />{/if}
        <div class="story-body">
          <div class="story-meta">{blog.source || blog.theme || ''} · {blog.category || ''}</div>
          <h3 class="story-title">{blog.title}</h3>
        </div>
        <div class="story-actions">
          <span class="stat-rank">#{i + 1}</span>
          <span class="story-views">{mockViews[i].toLocaleString()} views</span>
        </div>
      </div>
    {/each}
  </div>
