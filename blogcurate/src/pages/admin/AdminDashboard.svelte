<script>
  ...
  import { pipelineStatus, fetchStatus, currentAdminPage, runHistory, fetchRuns } from '../../stores/store.js';

  let interval;
  onMount(() => { fetchStatus(); fetchRuns(); interval = setInterval(() => { if ($pipelineStatus?.status === 'RUNNING') fetchStatus(); }, 30000); });
...
<div class="db-root">
  ...
  <div class="welcome-row">
    <div>
      ...
      <p class="welcome-sub">Your AI content engine curated <strong>48 new articles</strong> today.</p>
    </div>
    {#if isRunning}
      <div class="running-pill">
        <span class="pulse-dot"></span> Pipeline Running
      </div>
    {/if}
  </div>
  ...
  <div class="stat-grid">
    <div class="stat-card">
      ...
      <div class="stat-name">Blogs Curated Today</div>
      <div class="stat-delta">↑ +12% from yesterday</div>
    </div>
    <div class="stat-card">
      <div class="stat-icon">📄</div>
      <div class="stat-val">1,254</div>
      <div class="stat-name">Articles Published</div>
      <div class="stat-delta">↑ +8% this month</div>
    </div>
    <div class="stat-card">
      <div class="stat-icon">🎯</div>
      <div class="stat-val">94.7%</div>
      <div class="stat-name">AI Accuracy Score</div>
      <div class="stat-delta">High Quality</div>
    </div>
    <div class="stat-card">
      <div class="stat-icon">🔌</div>
      <div class="stat-val">126</div>
      <div class="stat-name">Active Sources</div>
      <div class="stat-delta">RSS · Medium · News APIs</div>
    </div>
  </div>
  ...
  <div class="db-card table-card">
    <div class="card-head">
      <div>
        <div class="card-title">Recent Curated Blogs</div>
        <div class="card-sub">Latest articles processed by AI</div>
      </div>
    ...
    <div class="table-wrap">
      <table class="blogs-table">
        <thead>
          <tr>
            <th>Blog Title</th><th>Source</th><th>AI Score</th>
            <th>Category</th><th>Status</th><th>Publish Date</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
          {#each blogs as b}
            <tr>
              ...
              <td class="td-source">{b.source}</td>
              <td><span class="score-pill">{b.score}</span></td>
              <td><span class="cat-tag">{b.category}</span></td>
              <td><span class="badge {statusMeta[b.status].cls}">{statusMeta[b.status].label}</span></td>
