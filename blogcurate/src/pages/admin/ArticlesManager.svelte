<script>
  ...
  // Mock articles for table display
  const mockArticles = [
    { id:'m1', title:'GPT-5 Is Changing How Developers Work', category:'AI',       source:'TechCrunch',  score:97, date:'10 Jul 2025', status:'Published', thumb:'🤖' },
    { id:'m2', title:'The Rise of Agentic AI in Enterprise',  category:'AI',       source:'Wired',        score:91, date:'10 Jul 2025', status:'Published', thumb:'🧠' },
    { id:'m3', title:'Top 10 Data Analytics Tools 2025',      category:'Data',     source:'Medium',       score:85, date:'11 Jul 2025', status:'Scheduled', thumb:'📊' },
    { id:'m4', title:'Remote Work is Not Dead — It Evolved',  category:'Work',     source:'Forbes',       score:78, date:'09 Jul 2025', status:'Review',    thumb:'💼' },
    { id:'m5', title:'Zero Trust Security in 2025',           category:'Cyber',    source:'Dark Reading', score:88, date:'09 Jul 2025', status:'Published', thumb:'🔒' },
    { id:'m6', title:'Why Startup Valuations Are Rebounding', category:'Finance',  source:'Bloomberg',    score:62, date:'08 Jul 2025', status:'Rejected',  thumb:'💹' },
    { id:'m7', title:'LLMs and the Future of Code Review',    category:'AI',       source:'Dev.to',       score:93, date:'08 Jul 2025', status:'Draft',     thumb:'⌨️' },
  ];

  const statusTabs = ['all','Published','Draft','Scheduled','Review','Rejected'];
  const statusBadgeMap = {
    Published: 'badge-complete',
    Scheduled: 'badge-running',
    Review:    'badge-amber',
    Rejected:  'badge-error',
    Draft:     'badge-gray',
  };

  $: visibleMock = activeStatus === 'all'
    ? mockArticles.filter(a => !searchQuery || a.title.toLowerCase().includes(searchQuery.toLowerCase()))
    : mockArticles.filter(a => a.status === activeStatus && (!searchQuery || a.title.toLowerCase().includes(searchQuery.toLowerCase())));

  $: mockSelected = new Set([...selected].filter(id => mockArticles.some(a => a.id === id)));
</script>
...
<div class="articles-page" class:has-slide={slideOver}>
  ...
  <div class="art-stats">
    {#each [
      { label:'Total Articles', val: mockArticles.length },
      { label:'Published',      val: mockArticles.filter(a=>a.status==='Published').length },
      { label:'Scheduled',      val: mockArticles.filter(a=>a.status==='Scheduled').length },
      { label:'Drafts',         val: mockArticles.filter(a=>a.status==='Draft').length },
    ] as s}
      <div class="art-stat">
              ...
              <tr class:row-selected={selected.has(a.id)}>
                ...
                <td>
                  <div class="act-row">
                    ...
                    <button class="act-btn act-pub" title="Publish Now">
  ...
  {:else if filtered.length > 0}
    <div style="margin-top:24px;border-top:1px solid var(--divider);padding-top:20px">
      <div style="font-size:13px;font-weight:600;color:var(--text-muted);margin-bottom:12px;text-transform:uppercase;letter-spacing:0.4px;">Live Articles ({filtered.length})</div>
